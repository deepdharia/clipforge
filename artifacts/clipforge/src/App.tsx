...
const DEMO_CLIPS: Clip[] = [
  { id: 1, title: 'Clip 01 · 0:04', start: 4, end: 18, status: 'ready' },
  { id: 2, title: 'Clip 02 · 0:18', start: 18, end: 33, status: 'ready' },
  { id: 3, title: 'Clip 03 · 0:33', start: 33, end: 47, status: 'ready' },
];
    ...
    <div className="cf-clip-row" data-testid={`row-clip-${clip.id}`}>
      ...
      <div className="cf-clip-copy">
        ...
        <span className={`cf-clip-status ${clip.status === 'exporting' ? 'pending' : ''}`} data-testid={`status-clip-${clip.id}`}>
          <span /> {clip.status === 'exporting' ? 'Processing...' : 'Ready'}
        </span>
      </div>
      <div>
  ...
  return (
    <aside className="cf-inspector" data-testid="panel-inspector">
      <section className="cf-inspector-section">
        ...
        <p className="cf-helper">Video will be centered and cropped to your selected ratio.</p>
      </section>
      ...
      <section className="cf-inspector-section">
        ...
        {splitMode === 'preset' ? (
          ...
          <div className="cf-duration-row">
            <label className="cf-helper" htmlFor="custom-duration">Length (seconds)</label>
            ...
            <span className="cf-helper">sec</span>
          </div>
        )}
        <p className="cf-helper">Split your selected range into equal-length clips.</p>
      ...
      <section className="cf-inspector-section" data-testid="panel-clip-queue">
        <div className="cf-queue-head">
          <div className="cf-section-heading" style={{ marginBottom: 0 }}><h3>Clips</h3></div>
        ...
        {clips.length > 0 ? (
          <>
            ...
            <button className="cf-outline-btn" style={{ width: '100%', justifyContent: 'center', marginTop: 11 }} onClick={onDownloadAll} data-testid="button-download-all">
              <ZipDownload size={14} /> Download all as ZIP
            </button>
          </>
        ) : (
          <div className="cf-queue-empty" data-testid="empty-clip-queue">Select a range and split to create clips.</div>
        )}
      </section>
    </aside>
  );
...
function Home() {
  ...
  const [fileName, setFileName] = useState('sample-video.mp4');
  ...
  useEffect(() => {
    ...
    if (isPlaying) {
      void video.play().catch(() => {
        setIsPlaying(false);
        notify('Playback blocked by browser. Try again.');
      });
    } else {
      video.pause();
    }
  ...
  const handleFile = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    setSourceDuration(1);
    setCurrentTime(0);
    notify('Video loaded. Ready to split.');
  };
  ...
  const splitClips = () => {
    if (!hasVideo || rangeEnd <= rangeStart) {
      notify('Select a valid range first.');
      return;
    }
    const next: Clip[] = [];
    ...
    setClips(next);
    notify(`${next.length} clip${next.length === 1 ? '' : 's'} created.`);
  };
  ...
  const renderClip = async (clip: Clip) => {
    ...
    if (!video || !videoUrl || !videoFile) {
      throw new Error('Upload a video first.');
    }
    if (!video.videoWidth || !video.videoHeight) {
      throw new Error('Video is still loading. Try again.');
    }
    if (!HTMLCanvasElement.prototype.captureStream || typeof MediaRecorder === 'undefined') {
      throw new Error('Your browser does not support in-browser video export.');
    }
    ...
    if (!context) throw new Error('Could not prepare canvas.');
  ...
  const downloadClip = async (id: number) => {
    ...
    try {
      ...
      triggerDownload(url, `split-video-${String(id).padStart(2, '0')}.webm`);
      notify(`Clip ${id} downloaded.`);
    } catch (error) {
  ...
  return (
    <div className="cf-app">
      <header className="cf-topbar">
        <div className="cf-brand" data-testid="text-brand"><span className="cf-brand-mark"><Scissors size={17} strokeWidth={2.5} /></span>SplitVideo</div>
        <div className="cf-project">
          <span className="cf-project-name" data-testid="text-project-name">{fileName}</span>
          <span className="cf-project-status" data-testid="status-project"><span /> Saved</span>
        </div>
        <div className="cf-top-actions">
          <IconButton label="Undo" testId="button-undo" onClick={undo}><Undo2 size={16} /></IconButton>
          <IconButton label="Redo" testId="button-redo" onClick={() => notify('Nothing to redo.')}><Redo2 size={16} /></IconButton>
          <IconButton label="Help" testId="button-help" onClick={() => setHelpOpen((open) => !open)}><CircleHelp size={17} /></IconButton>
          <IconButton label="Settings" testId="button-settings" onClick={() => setSettingsOpen((open) => !open)}><Settings2 size={16} /></IconButton>
        </div>
        {helpOpen && (
          <div className="cf-settings" data-testid="panel-help">
            <h3><MessageCircleQuestion size={14} style={{ verticalAlign: 'middle', marginRight: 6 }} /> Help</h3>
            <p>1. Upload a video or load the demo</p>
            <p>2. Select a time range on the timeline</p>
            <p>3. Choose a split duration and click "Split into clips"</p>
            <p>4. Download individual clips or all at once</p>
            <p style={{ fontSize: 12, color: '#999', marginTop: 12 }}>All processing happens in your browser. No server uploads.</p>
            <button className="cf-outline-btn" style={{ width: '100%', justifyContent: 'center', marginTop: 12 }} onClick={() => setHelpOpen(false)} data-testid="button-close-help">Close</button>
          </div>
        )}
        {settingsOpen && (
          <div className="cf-settings" data-testid="panel-settings">
            <h3>Settings</h3>
            <p>All your video data stays in your browser. Nothing is uploaded to our servers.</p>
            <div className="cf-settings-row"><span>Output format</span><strong style={{ color: '#c7ed79', fontSize: 10 }}>WebM</strong></div>
      ...
      <nav className="cf-rail" aria-label="Workspace tools">
        ...
        <button className="cf-rail-button" title="Media" data-testid="button-nav-media"><FolderOpen size={18} /></button>
        ...
        <div className="cf-rail-bottom">
          <button className="cf-rail-button" title="History" data-testid="button-nav-history"><History size={18} /></button>
          <button className="cf-rail-button" title="Help" data-testid="button-nav-help" onClick={() => setHelpOpen(true)}><HelpCircle size={18} /></button>
        </div>
      </nav>

      <main className="cf-main">
        <div className="cf-workspace-header">
          <div className="cf-breadcrumb"><span className="hidden sm:inline">Workspace</span><span className="cf-breadcrumb-sep">/</span><strong data-testid="text-page-title">Split video into clips</strong></div>
        ...
        <div className="cf-editor-grid">
          <section className="cf-workspace">
            <div className="cf-preview-card" data-testid="panel-preview">
              <div className="cf-preview-toolbar">
                ...
                <div className="cf-preview-actions">
                  <IconButton label="Fit" testId="button-fit-preview"><Maximize2 size={14} /></IconButton>
                  <IconButton label="Options" testId="button-preview-options"><MoreHorizontal size={16} /></IconButton>
                </div>
              </div>
                  ...
                  <>
                     <div className="cf-video-frame" style={{ aspectRatio: aspect === '16:9' ? '16/9' : aspect === '1:1' ? '1/1' : '9/16' }} data-testid="video-preview-frame">
                       {videoUrl ? (
                         ...
                         <div className="cf-frame-caption"><small>Sample video</small>Ready to split</div>
                       )}
                    </div>
                     <span className="cf-stage-time" data-testid="text-preview-time">{formatTime(currentTime)} / {formatTime(sourceDuration)}</span>
                    <span className="cf-stage-badge" data-testid="status-preview">{isDemo ? 'DEMO' : 'READY'}</span>
                  </>
                ) : (
              ...
              <div className="cf-transport">
                <button className="cf-play" onClick={() => setIsPlaying((playing) => !playing)} data-testid="button-play-preview" aria-label={isPlaying ? 'Pause' : 'Play'}>
                 ...
                 />
                 <div className="cf-volume"><Volume2 size={14} /><input ref={(element) => { if (element && videoRef.current) videoRef.current.volume = Number(element.value) / 100; }} type="range" min="0" max="100" defaultValue="100" data-testid="input-volume" /></div>
              </div>
            </div>

            <div className="cf-timeline" data-testid="panel-timeline">
              <div className="cf-timeline-head">
                <span className="cf-label"><span className="dot" style={{ background: '#ff786d' }} /> Timeline</span>
              ...
              <div className="cf-timeline-body">
                ...
                <div className="cf-track" data-testid="timeline-track">
                 ...
                 <input className="cf-range-input" type="range" min="0" max={Math.max(sourceDuration - 0.01, 0.01)} step="0.01" value={Math.min(rangeStart, Math.max(sourceDuration - 0.01, 0.01))} onChange={(event) => setRangeStart(Number(event.target.value))} aria-label="Range start" data-testid="input-range-start" />
                 <input className="cf-range-input" type="range" min="0.01" max={sourceDuration} step="0.01" value={Math.max(rangeEnd, 0.01)} onChange={(event) => setRangeEnd(Math.max(Number(event.target.value), 0.01))} aria-label="Range end" data-testid="input-range-end" />
           ...
           <Inspector aspect={aspect} setAspect={setAspect} splitMode={splitMode} setSplitMode={setSplitMode} duration={duration} setDuration={setDuration} clips={clips} onSplit={splitClips} onDownloadAll={downloadAll} onDownload={downloadClip} onDelete={(id) => setClips(clips.filter((c) => c.id !== id))} />
        </div>
      </main>
